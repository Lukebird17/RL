...
base_checkpoint_path = "/home/robot/SERL/serl/serl_checkpoints/pap"
checkpoint_path = os.path.join(base_checkpoint_path, date_time_str)
# CHECKPOINT_PATH = "/home/robot/SERL/serl/serl_checkpoints/pap/"
FLAGS = flags.FLAGS
...
def load_agent(checkpoint_dir, env):
    ...
    if latest_checkpoint:
        agent = agent.replace(state=agent.state.replace(params=latest_checkpoint["params"]))
        print("Checkpoint loaded successfully from:", checkpoint_dir)
    else:
        raise ValueError("No checkpoint found in the given directory.")
